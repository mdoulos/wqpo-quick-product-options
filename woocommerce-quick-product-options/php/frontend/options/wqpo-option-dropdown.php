...
?>
<select name="wqpo-<?= $option_count ?>" class="wqpo-select" onchange="wqpoChangeSelect()">
    <?php
    // Output the Choices
    for ($choice_count = 0; $choice_count < $total_choice_count; $choice_count++) {
        ...
        if ( isset($choice_skus[$choice_count]) && $choice_skus[$choice_count] != '' ) { 
            $sku_text = $setting_skulabel . $setting_skulabelsuffix . ' '. $choice_skus[$choice_count] . $setting_skusuffix . ' '; }
        else { $sku_text = ''; }

        // Output the Choice Text ?>
        <option class="<?= 'wqpo-choice' . $choice_order ?><?= $modifies_price . $tags ?>" value="<?= $choice_count ?>" data-price="<?= $choice_prices[$choice_count] ?>"<?php echo $showtags . $hidetags . $enable_modifiers . $choice_modifiers_string; ?> data-type="<?= $option_types[$option_count] ?>"><?= $sku_text . $choice_names[$choice_count] ?><?php if ($choice_prices[$choice_count] > 0) { echo '$' . $choice_prices[$choice_count]; } ?><?php if ($choice_descriptions[$choice_count]) { echo $choice_descriptions[$choice_count]; } ?></option>
        <?php 
    } ?>
</select>