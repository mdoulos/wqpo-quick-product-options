...
for ($choice_count = 0; $choice_count < $total_choice_count; $choice_count++) {
    ...
    // Prepare SKU HTML
    if ( isset($choice_skus[$choice_count]) && $choice_skus[$choice_count] != '' ) { 
        $sku_html = '<span class="wqpo-skulabel">' . $setting_skulabel . $setting_skulabelsuffix . '</span><span class="wqpo-sku">' . $choice_skus[$choice_count] . '</span>' . $setting_skusuffix . ' '; }
    else { $sku_html = ''; }

    // Output the Choice HTML ?>
    <div class="<?= 'wqpo-choice' . $choice_order ?><?= $modifies_price . $tags ?>"<?php echo $showtags . $hidetags . $enable_modifiers . $choice_modifiers_string; ?> data-type="<?= $option_types[$option_count] ?>">
        <div class="flex-row">
            <label for="<?= $choice_id ?>">
                <span class="wqpo-choice-text"><?= $sku_html . $choice_names[$choice_count] ?></span>
                <span class="wqpo-choice-price" data-price="<?= $choice_prices[$choice_count] ?>">$<?= $choice_prices[$choice_count] ?></span>
            </label>
            <input type="radio" name="wqpo-<?= $option_count ?>"<?php if ($choice_count == 0) { echo $is_required; } ?> id="<?= $choice_id ?>" value="<?= $choice_count ?>" onchange="wqpoClickRadio()">
        </div>
        <?php if ( $choice_descriptions[$choice_count] ) { ?>
            <p class="wqpo-choice-description">
                <?= $choice_descriptions[$choice_count] ?>
            </p>
        <?php } ?>
    </div>
